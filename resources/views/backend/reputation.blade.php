...
@section('content')
                ...
                <div class="col-sm-12 col-md-12">
                    <div class="card">
                        <div class="card-header align-items-center justify-content-between d-flex">
                            ...
                            <button class="btn btn-primary btn-sm ReputationAddButton"
                                style="float: right;margin-top:-20px;"><i class="fa fa-plus"></i>Reputation Add</button>
                        </div>

                        <div class="card-block container">
                            <div class="table-responsive" id="tab">
                                <table class="table table-striped table-bordered table-hover"
                                    style="border: solid 1px rgba(255, 193, 193, 0.1);" id="Reputation_table">
                                </table>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="ReputationAdd" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <form id="Reputation_insert_update" action="{{ route('admin.account.reputation.insert') }}"
                accept-charset="utf-6" enctype="multipart/form-data" method="post" class="form-horizontal validatable">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="exampleModalLabel">Reputation Add</h1>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-6 mb-6">
                                <label for="recipient-name" class="col-form-label">Consultation Area Name:</label>
                                <input type="text" name="name" id="name" class="form-control">
                            </div>
                            <div class="col-6 mb-6">
                                <label for="recipient-name" class="col-form-label">Consultation Area Icon:</label>
                            ...
                            <div class="col-6 mb-6">
                                <label for="recipient-name" class="col-form-label">Consultation Area Details:</label>
                                <textarea name="law_details" class="form-control ckeditor" id="law_details" cols="30" rows="10"></textarea>
                            </div>
                            ...
                            <div class="form-group col-4 mb-2">
                                <label for="favicon">Upload Image<span class="text-danger">*</span> <span
                                        class="text-danger">( Size: 600*450)</span></label>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#Reputation_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.account.reputation.data') }}",
                    type: 'GET',
                    cache: false
                },
                columns: [{
            ...
            $('#Reputation_insert_update').ajaxForm({
                beforeSend: formBeforeSend,
                beforeSubmit: formBeforeSubmit,
                error: formError,
                success: function(responseText, statusText, xhr, $form) {
                    formSuccess(responseText, statusText, xhr, $form);
                    $('#Reputation_table').DataTable().draw(true);

                    $("#ReputationAdd").modal('hide');
            ...
            $(document).on('click', '.ReputationAddButton', function() {

                $("#customer_insert_update").trigger("reset");
                $('#hidden-id').attr("disabled", "true");
                $("#ReputationAdd").modal('show');
                var customerId = "T" + generator.ReputationAdd();
            ...
            $(document).on('click', '.tableDelete', function() {
                ...
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ route('admin.account.reputation.insert') }}",
                            type: "POST",
                            data: {
            ...
            $(document).on('click', '.tableEdit', function() {
                ...
                $(this).ajaxSubmit({
                    ...
                    dataType: 'json',
                    method: 'GET',
                    url: "{{ route('admin.account.reputation.edit') }}",
                    success: function(responseText) {
                      ...
                      tinymce.get('law_details').setContent(responseText.data.law_details);
                        ...
                        var newOption = new Option(responseText.legal_area_name,
                            responseText.data.legal_area_id, false, false);

                        $('select[name^="legal_area_id"]').empty().append(newOption);
            ...
            $('#ReputationAdd').on('hidden.bs.modal', function() {
                $('#Reputation_insert_update')[0].reset(); // Clear the form data
            });
